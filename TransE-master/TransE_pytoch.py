...
def dataloader(file1, file2, file3, file4):
    ...
    # 获得验证集的三元组集合
    valid_triple_list = []
    with codecs.open(file4, 'r', encoding='utf-8') as f:
        content = f.readlines()
        for line in content:
            triple = line.strip().split("\t")
            if len(triple) != 3:
...
class TransE:
    ...
    def insert_test_data(self, file1, file2, file3):
        ...
        triple_list = []
        with codecs.open(file3, 'r',encoding='utf-8') as f4:
...
if __name__ == '__main__':
    ...
    file2 = "./dataset/entity-id.txt"
    file3 = "./dataset/relation-id.txt"
    file4 = "./dataset/test.txt"

    file5 = "ZH_torch_TransE_entity_75dim_batch1024"
    file6 = "ZH_torch_TransE_relation_75dim_batch1024"

    file8 = "./dataset/test.txt"
    entity_set, relation_set, triple_list, valid_triple_list = dataloader(file1, file2, file3, file4)

    transE = TransE(entity_set, relation_set, triple_list, embedding_dim=75, lr=0.001, margin=4.0, norm=1, C = 0.25, valid_triple_list=valid_triple_list)
    transE.data_initialise()
    # transE.training_run(epochs=200, batch_size=1024, out_file_title="ZH_torch_")
